...
AUTHENTICATION_BACKEND="${AUTHENTICATION_BACKEND:-airflow.contrib.auth.backends.ldap_auth}"
...
sed -i "s#{{ ENABLE_AUTHENTICATION }}#${ENABLE_AUTHENTICATION}#" $AIRFLOW_HOME/airflow.cfg
sed -i "s#{{ AUTHENTICATION_BACKEND }}#${AUTHENTICATION_BACKEND}#" $AIRFLOW_HOME/airflow.cfg
sed -i "s#{{ LDAP_PROTOCOL }}#${LDAP_PROTOCOL}#" $AIRFLOW_HOME/airflow.cfg
